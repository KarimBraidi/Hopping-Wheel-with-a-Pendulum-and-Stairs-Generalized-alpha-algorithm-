...
class PlanarStairModel:
    ...
    Prediction model for the NMPC. Its free-flight dynamics are the exact
    planar restriction of the full quaternion simulator's Lagrangian
    (cross-checked numerically against compute_mass_matrix / compute_h):
    generalized coordinates [x, z, phi, alpha] with phi the shell angle
    about +y and alpha the pendulum angle relative to the shell. Only
    psi = alpha - phi (the pendulum's world angle) enters the equations,
    so the rollout state is

        s = [x, z, psi, vx, vz, phi_dot, alpha_dot]      (7 per sample)
    ...
    def __init__(self, m_sphere, m_pendulum, l_pendulum, I_sphere, radius,
        ...
        self._nz0 = None
        # constant Schur complement of the (x,z,phi,alpha) mass matrix
        # (see step()); precomputed once for the closed-form block solve
        ml2 = self.m_p * self.l_p * self.l_p
        k_s = (self.m_p * self.l_p) ** 2 / self.m_t
        S_blk = np.array([[self.I_s + ml2 - k_s, -ml2 + k_s],
                          [-ml2 + k_s, ml2 - k_s]])
        self.Sinv = np.linalg.inv(S_blk)
    ...
    def step(self, S, tau, dt):
        ...
        x, z, psi = S[:, 0], S[:, 1], S[:, 2]
        vx, vz, wp, wa = S[:, 3], S[:, 4], S[:, 5], S[:, 6]
        sn, cs = np.sin(psi), np.cos(psi)
        psid = wa - wp
        mpl = self.m_p * self.l_p

        # RHS = Q_applied + Q_gravity - Coriolis  (Coriolis only hits x,z rows)
        F = np.empty((S.shape[0], 4))
        F[:, 0] = mpl * cs * psid * psid
        F[:, 1] = mpl * sn * psid * psid - self.m_t * self.g
        F[:, 2] = self.m_p * self.g * self.l_p * cs
        ...
        for c_idx, (gap, nx, nz) in enumerate(self._contacts(x, z)):
            ...
            lam = np.where(pen > 0.0,
                           ...
                           0.0)
            # tangent t = (nz, -nx); slip of the material contact point
            slip = (vx - wp * self.R * nz) * nz - (vz + wp * self.R * nx) * nx
        ...
        # Closed-form block solve of M u_dot = F. In (x, z, phi, alpha)
        # velocity space the mass matrix is [[m_t I, B], [B^T, D]] with
        # B = mpl [[s, -s], [-c, c]]; because B^T B = mpl^2 [[1,-1],[-1,1]]
        # is state-INDEPENDENT, the Schur complement S = D - B^T B / m_t is
        # constant and its inverse is precomputed in __init__ — the whole
        # solve reduces to a few elementwise ops (several times faster than
        # a batched 4x4 np.linalg.solve, and this is the rollout hot loop).
        f1, f2 = F[:, 0], F[:, 1]
        e = mpl * (sn * f1 - cs * f2) / self.m_t
        gp1 = F[:, 2] - e
        gp2 = F[:, 3] + e
        w1 = self.Sinv[0, 0] * gp1 + self.Sinv[0, 1] * gp2
        w2 = self.Sinv[1, 0] * gp1 + self.Sinv[1, 1] * gp2
        dw = mpl * (w1 - w2)
        v1 = (f1 - dw * sn) / self.m_t
        v2 = (f2 + dw * cs) / self.m_t
...
class StairNMPCController:
    ...
    def _planar_state(self, q, u):
        ...
        return np.array([q[0], q[2], psi, u[0], u[2], u[4], u[6]])
