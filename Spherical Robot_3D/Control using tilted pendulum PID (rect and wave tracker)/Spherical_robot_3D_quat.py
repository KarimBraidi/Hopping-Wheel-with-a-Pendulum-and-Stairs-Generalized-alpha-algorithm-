...
class Simulation:
    def __init__(
        ...
        # torque-controlled pendulum path tracking, all inputs in SI
        control_rect=None,        # (Lx, Ly) rectangle dimensions [m], or None
        control_sine=None,        # dict or tuple defining y = yc + A*sin(2*pi*(x-x0)/wavelength + phase)
        tau_max=1.0,              # joint torque saturation [N*m]
        kp_pend=80.0,             # virtual spring at pendulum tip [N/m]
        kd_pend=8.0,              # virtual damper at pendulum tip [N*s/m]
        kd_joint=0.10,            # joint-space damping  [N*m*s/rad]
        v_max=0.4,                # cruise speed along the reference path [m/s]
        k_v=2.0,                  # velocity-loop gain [1/s]
        k_pos=1.0,                # waypoint approach gain [1/s]
        k_path=2.0,               # cross-track correction gain [1/s]
        k_path_d=1.2,             # cross-track velocity damping [-]
        a_cmd_max=1.0,            # max commanded acceleration [m/s^2]
        lean_max=np.deg2rad(30),  # max pendulum lean from vertical [rad]
        waypoint_tol=0.05,        # waypoint switch radius [m]
        save_interval=100,        # write arrays every N accepted steps
        log_interval=1,           # print/write progress every N accepted steps
    ):
        # ── output directory ──────────────────────────────────
        ...
        # ── torque-controlled pendulum path tracking ──────────
        has_rect_control = control_rect is not None
        has_sine_control = control_sine is not None
        if has_rect_control and has_sine_control:
            raise ValueError("Choose only one path control mode: control_rect or control_sine.")

        self.control_enabled = has_rect_control or has_sine_control
        ...
        if self.control_enabled and (self.prescribed_alpha or self.prescribed_beta):
            raise ValueError("Cannot combine torque control with prescribed pendulum motion.")
        # unit conversions to code (nondimensional) units
        F_nd = m_nd * a_nd                                  # force scale [N]
        T_nd = F_nd * l_nd                                  # torque scale [N*m]
        self.tau_max = float(tau_max) / T_nd                # torque
        self.kp_pend = float(kp_pend) * l_nd / F_nd         # tip stiffness  [N/m]
        self.kd_pend = float(kd_pend) * l_nd / (F_nd * t_nd)  # tip damping  [N*s/m]
        self.kd_joint = float(kd_joint) / (T_nd * t_nd)     # joint damp  [N*m*s/rad]
        self.v_max = float(v_max) * t_nd / l_nd             # velocity
        self.k_v = float(k_v) * t_nd                        # 1/time
        self.k_pos = float(k_pos) * t_nd                    # 1/time
        self.k_path = float(k_path) * t_nd                  # 1/time
        self.k_path_d = float(k_path_d)                     # dimensionless
        self.a_cmd_max = float(a_cmd_max) / a_nd            # acceleration
        self.lean_max = float(lean_max)
        self.waypoint_tol = float(waypoint_tol) / l_nd
        self.save_interval = max(1, int(save_interval))
        self.log_interval = max(1, int(log_interval))
        if self.control_enabled:
            x0c, y0c = float(x0) / l_nd, float(y0) / l_nd
            if has_rect_control:
                self.control_mode = "rectangle"
                Lx, Ly = float(control_rect[0]) / l_nd, float(control_rect[1]) / l_nd
                ...
                self.control_mode = "sine"
                self.sine_params = self._parse_sine_config(control_sine, x0c, y0c, l_nd)
                self.path_points = self._build_sine_path(self.sine_params)

            path_seg = np.diff(self.path_points, axis=0)
            self.path_segment_lengths = np.linalg.norm(path_seg, axis=1)
            self.path_s = np.concatenate(([0.0], np.cumsum(self.path_segment_lengths)))
            self.path_total_length = float(self.path_s[-1])

            # Reference points after the initial point. For rectangles these are
            # corners; for sine paths they are samples along the curve.
            self.waypoints = self.path_points[1:].copy()
            self.wp_idx = 0
        ...
        self.tau_save = np.zeros((2, self.ntime))
    ...
    def _save_metadata(self):
        params = {
            ...
            "dtime": float(self.dtime), "dt": float(self.dtime * np.sqrt(1.0 / 9.81)), "ntime": self.ntime,
            "R": float(self.R),
            ...
            "sine_params": self.sine_params,
            "path_total_length": float(self.path_total_length),
            "tau_max": float(self.tau_max),
            "k_path": float(self.k_path),
            "k_path_d": float(self.k_path_d),
            "save_interval": int(self.save_interval),
            "log_interval": int(self.log_interval),
            "waypoints": self.waypoints.tolist() if self.control_enabled else None,
        }
        with open(os.path.join(self.output_path, "params.json"), "w") as fp:
            json.dump(params, fp, indent=2)

    def _parse_sine_config(self, control_sine, x0c, y0c, l_nd):
        """Return a normalized sine-path config in code units.

        Accepted forms:
          control_sine=(length, amplitude, wavelength)
          control_sine=(length, amplitude, wavelength, phase)
          control_sine=(length, amplitude, wavelength, phase, num_points)
          control_sine={
              "length": ..., "amplitude": ..., "wavelength": ...,
              "phase": 0.0, "y_center": y0, "num_points": 401
          }

        If a dict gives "cycles" instead of "wavelength", wavelength is
        computed as length / cycles.
        """
        if isinstance(control_sine, dict):
            raw = dict(control_sine)
            ...
            x_start_value = pick("x_start", "start_x", "x0", default=float(x0c * l_nd))
            x_start = float(x_start_value) / l_nd

            length_value = pick("length", "x_length", "x_span", "L", default=None)
            if length_value is None:
                x_end_value = pick("x_end", "end_x", default=None)
                if x_end_value is None:
                    raise ValueError("control_sine dict requires 'length' or 'x_end'.")
                length = float(x_end_value) / l_nd - x_start
            else:
                length = float(length_value) / l_nd

            amplitude = float(pick("amplitude", "amp", "A", default=0.0)) / l_nd

            wavelength_value = pick("wavelength", "period", "lambda", default=None)
            cycles_value = pick("cycles", "n_cycles", default=None)
            if wavelength_value is None:
                cycles = 1.0 if cycles_value is None else float(cycles_value)
                if cycles <= 0.0:
                    raise ValueError("control_sine cycles must be positive.")
                wavelength = length / cycles
            else:
                wavelength = float(wavelength_value) / l_nd
                cycles = length / wavelength if abs(wavelength) > 1e-12 else np.inf

            phase = float(pick("phase", "phase_rad", default=0.0))

            y_center_value = pick("y_center", "center_y", "yc", default=None)
            if y_center_value is None:
                y_offset = float(pick("y_offset", "offset_y", default=0.0)) / l_nd
                y_center = y0c + y_offset
            else:
                y_center = float(y_center_value) / l_nd

            samples_per_cycle = int(pick("samples_per_cycle", default=120))
            num_points_value = pick("num_points", "n_points", "points", "samples", default=None)
            if num_points_value is None:
                num_points = int(max(80, abs(cycles) * samples_per_cycle)) + 1
            else:
                num_points = int(num_points_value)
        else:
            try:
                values = tuple(control_sine)
            except TypeError as exc:
                raise ValueError(
                    "control_sine must be a dict or tuple: "
                    "(length, amplitude, wavelength[, phase[, num_points]])."
                ) from exc
            if not (3 <= len(values) <= 5):
                raise ValueError(
                    "control_sine tuple must be "
                    "(length, amplitude, wavelength[, phase[, num_points]])."
                )
            length = float(values[0]) / l_nd
            amplitude = float(values[1]) / l_nd
            wavelength = float(values[2]) / l_nd
            phase = float(values[3]) if len(values) >= 4 else 0.0
            cycles = length / wavelength if abs(wavelength) > 1e-12 else np.inf
            num_points = int(values[4]) if len(values) >= 5 else int(max(80, abs(cycles) * 120)) + 1
            x_start = x0c
            y_center = y0c

        if length <= 0.0:
            raise ValueError("control_sine length must be positive.")
        if wavelength <= 0.0:
            raise ValueError("control_sine wavelength must be positive.")
        if num_points < 2:
            raise ValueError("control_sine num_points must be at least 2.")
    ...
    def _build_sine_path(self, sine_params):
        xs = np.linspace(
            sine_params["x_start"],
            sine_params["x_start"] + sine_params["length"],
            sine_params["num_points"],
        )
        ys = (
            sine_params["y_center"]
            + sine_params["amplitude"]
            * np.sin(
                2.0 * np.pi * (xs - sine_params["x_start"]) / sine_params["wavelength"]
                + sine_params["phase"]
            )
        )
    ...
    def save_arrays(self):
        ...
        np.save(f'{self.output_path}/tau_save.npy', self.tau_save)
    ...
    # ══════════════════════════════════════════════════════════
    #  Pendulum torque controller (path tracking)
    # ══════════════════════════════════════════════════════════
    def _pendulum_control_torque(self, q, u):
        """Joint torques (tau_alpha, tau_beta) in code units.

        Cascade:
          1. path segment -> desired sphere velocity (capped at v_max)
          2. velocity error -> commanded acceleration (capped at a_cmd_max,
             keeps the lean shallow so the sphere never jumps)
          3. acceleration -> desired pendulum direction in the WORLD frame
             (lean towards a_cmd; angle prop. to |a_cmd|, capped at lean_max)
          4. The joint damper is a passive sink that prevents that runaway 
             without fighting the spatial goal.
          5. saturate at +-tau_max.
        """
        if not self.control_enabled:
            return np.zeros(2)

        # outer loops: follow the active path segment, with a separate
        # damped cross-track correction. This avoids aiming through each
        # waypoint and reduces lateral hunting on short path segments.
        A = self.path_points[self.wp_idx]
        B = self.path_points[self.wp_idx + 1]
        edge = B - A
        edge_len = np.linalg.norm(edge)
        if edge_len > 1e-12:
            t_hat = edge / edge_len
            s = (q[0:2] - A) @ t_hat
            s_clamped = np.clip(s, 0.0, edge_len)
            closest = A + s_clamped * t_hat
            cross_err = closest - q[0:2]
            v_xy = u[0:2]
            v_cross = v_xy - (v_xy @ t_hat) * t_hat
            if self.control_mode == "sine":
                path_s_here = self.path_s[self.wp_idx] + s_clamped
                remaining = max(self.path_total_length - path_s_here, 0.0)
            else:
                remaining = max(edge_len - s, 0.0)
            v_along = min(self.v_max, self.k_pos * remaining)
            v_des = v_along * t_hat + self.k_path * cross_err - self.k_path_d * v_cross
            v_n = np.linalg.norm(v_des)
            if v_n > self.v_max:
                v_des = v_des * (self.v_max / v_n)
        else:
            v_des = np.zeros(2)
        a_cmd = self.k_v * (v_des - u[0:2])
        a_n = np.linalg.norm(a_cmd)
        if a_n > self.a_cmd_max:
            a_cmd = a_cmd * (self.a_cmd_max / a_n)
            a_n = self.a_cmd_max

        # desired pendulum direction (world frame)
        if a_n > 1e-12 and self.a_cmd_max > 0:
            phi = self.lean_max * (a_n / self.a_cmd_max)
            psi = np.arctan2(a_cmd[1], a_cmd[0])
        else:
            phi = 0.0
            psi = 0.0
        e_des = np.array([np.sin(phi)*np.cos(psi),
                          np.sin(phi)*np.sin(psi),
                          -np.cos(phi)])

        # virtual spring-damper at pendulum tip, J^T mapping
        Rm = quat_to_R(quat_normalize(q[3:7]))
        alpha_p, beta_p = q[7], q[8]
        ca, sa = np.cos(alpha_p), np.sin(alpha_p)
        cb, sb = np.cos(beta_p),  np.sin(beta_p)
        l = self.l_pendulum
        d = l * np.array([ca*cb, ca*sb, sa])
        dd_da = l * np.array([-sa*cb, -sa*sb, ca])
        dd_db = l * np.array([-ca*sb,  ca*cb, 0.0])
        x_rel = Rm @ d                      # tip position relative to centre
        xdot_rel = Rm @ (np.cross(u[3:6], d) + dd_da*u[6] + dd_db*u[7])
        f = self.kp_pend * (l*e_des - x_rel) - self.kd_pend * xdot_rel
        tau = np.array([(Rm @ dd_da) @ f, (Rm @ dd_db) @ f])

        # joint-space damping
        tau -= self.kd_joint * np.array([u[6], u[7]])

        # saturate
        return np.clip(tau, -self.tau_max, self.tau_max)

    def _advance_waypoint(self, q):
        """Switch to the next path segment after passing its end."""
        if not self.control_enabled or self.wp_idx >= len(self.waypoints) - 1:
            return
        advanced = False
        while self.wp_idx < len(self.waypoints) - 1:
            ...
            progress = (q[0:2] - A) @ (edge / edge_len)
            reached_segment_end = progress >= edge_len
            if self.control_mode == "rectangle":
                reached_segment_end = (
                    reached_segment_end
                    and np.linalg.norm(B - q[0:2]) < self.waypoint_tol
                )

            if not reached_segment_end:
                break

            self.wp_idx += 1
            advanced = True

        if advanced and self.control_mode == "rectangle":
            print(f"    >>> waypoint reached, now targeting corner "
                  f"{self.wp_idx}: {self.waypoints[self.wp_idx]}")
    ...
    def get_R(self, iter, X, prev_X, prev_AV, prev_q, prev_u, prev_gNdot, prev_gammaF):
        ...
        # ── Applied joint torques (pendulum control) ──────────
        tau_applied = np.zeros(self.nu)
        if self.control_enabled:
            tau_pc = self._pendulum_control_torque(q, u)
            tau_applied[6] = tau_pc[0]
            tau_applied[7] = tau_pc[1]
        ...
        # Smooth residual
        Rs = np.concatenate((
            M @ a + h - tau_applied - Wg @ lambda_g - Wgamma @ lambda_gamma - WN @ lambdaN - WF @ lambdaF,
    ...
    def time_update(self, iter):
        ...
        if self.control_enabled:
            self.tau_save[:, iter] = self._pendulum_control_torque(q, u)
            self._advance_waypoint(q)
    ...
    def plot_results(self):
        ...
        if self.control_enabled:
            ax = axes[2, 1]
            ax.plot(self.t, self.tau_save[0, :], 'b-', linewidth=1.2, label='τ_α')
            ax.plot(self.t, self.tau_save[1, :], 'r-', linewidth=1.2, label='τ_β')
            ax.axhline(self.tau_max, color='k', linestyle='--', alpha=0.4)
            ax.axhline(-self.tau_max, color='k', linestyle='--', alpha=0.4)
            ax.set_xlabel('Time'); ax.set_ylabel('τ (code units)')
            ax.set_title('Pendulum joint torques'); ax.legend(fontsize=8)
            ax.grid(True, alpha=0.3)
            ax = axes[2, 2]
            ax.plot(self.q_save[0, :], self.q_save[1, :], 'b-', linewidth=1.5, label='trajectory')
            ax.plot(
                self.path_points[:, 0], self.path_points[:, 1],
                'k--', linewidth=1.0, label=f'{self.control_mode} reference'
            )
            if self.control_mode == "rectangle":
                ax.plot(self.waypoints[:, 0], self.waypoints[:, 1], 'ro', markersize=5)
            else:
                ax.plot(self.path_points[0, 0], self.path_points[0, 1], 'go', markersize=5, label='start')
                ax.plot(self.path_points[-1, 0], self.path_points[-1, 1], 'ro', markersize=5, label='end')
            ax.set_xlabel('x'); ax.set_ylabel('y'); ax.set_title('XY tracking')
...
if __name__ == "__main__":
    # Torque-controlled pendulum: sphere tracks an XY reference path on flat ground.
    sim = Simulation(
        ntime=5000,
        dt=4e-3,
        mu_s=10**9, mu_k=0.5,
        ...
        p0_0=1.0, p1_0=0.0, p2_0=0.0, p3_0=0.0,
        alpha_p0=-np.pi/2 , beta_p0=0.0,  
        n_stairs=0,
        # Path tracking via joint torques.
        # Set exactly one path mode.
        # control_rect=(3.0, 1.0),  # Lx x Ly [m]
        #control_rect=None,
        control_sine={
            "length": 4.0,         # x distance [m]
            "amplitude": 1.0,      # y amplitude [m]
            "wavelength": 1.0,     # x wavelength [m]
            "phase": 0.0,          # phase [rad]
            "y_offset": 0.0,       # centre line relative to initial y [m]
            "num_points": 401,     # reference samples along the sine
         },
        # control_sine=None,
        tau_max=1.0,              # [N*m] 
        kp_pend=150.0,            # [N/m]   tip-space spring
        kd_pend=12.0,             # [N*s/m] tip-space damper
        kd_joint=0.10,            # [N*m*s/rad] joint damping
        v_max=0.45,                # [m/s]   cruise speed
        k_v=5.0, k_pos=1.2,       #
        k_path=8.0,               # [1/s] cross-track correction
        k_path_d=1.5,             # [-] lateral damping
        a_cmd_max=1.8,            # [m/s^2]
        lean_max=np.deg2rad(35),  #
        waypoint_tol=0.05,        # [m]
        save_interval=200,
        log_interval=1,
    )
