...
def smooth_limit(value, limit):
    """Smooth saturation with bounded slope around the limit."""
    lim = abs(float(limit))
    if lim <= 0.0:
        return 0.0
    return lim * np.tanh(float(value) / lim)
...
class Simulation:
    def __init__(
        ...
        # applied pendulum torques (generalized forces on alpha/beta rates)
        tau_alpha_func=None, tau_beta_func=None,
        # built-in torque profile (used when tau_*_func are not provided)
        tau_profile_mode="none",  # "none" or "smooth_step"
        tau_alpha_u0=0.0, tau_alpha_u1=0.0,
        tau_alpha_t0=0.0, tau_alpha_t1=1.0,
        tau_beta_u0=0.0, tau_beta_u1=0.0,
        tau_beta_t0=0.0, tau_beta_t1=1.0,
        # MPC-style convenience knobs: define only these two and a
        # smooth-step alpha torque is generated automatically.
        mpc_step_distance=None,
        mpc_step_duration=None,
        # Which axis the MPC-style step acts on. 'x' drives the alpha DOF
        # (default); 'y' drives the beta DOF. Note: for effective 'y' motion
        # the pendulum geometry should permit beta torque to translate the
        # sphere (e.g., set alpha_p0 != pi/2 or spin via beta_p_dot0).
        mpc_step_axis="x",
        # torque saturation (applies to both alpha and beta torques)
        torque_limit=np.inf,
        # pendulum viscous damping (prevents unbounded spin-up under
        # continuous internal torque; mirrors b_pend_mpc in MPC notebook)
        pendulum_damping=0.0,
        # True receding-horizon MPC (mirrors Simplified 2D Controllers).
        mpc_enable=False,
        mpc_trajectory_type="smooth_step",  # "sinusoid" or "smooth_step"
        ...
        mpc_guard_kp=6.0,
        mpc_guard_kd=8.0,
        mpc_guard_margin=0.05,
        # terminal hold mode (hysteresis) to avoid near-target chatter
        mpc_hold_enable=True,
        mpc_hold_err_enter=0.06,
        mpc_hold_err_exit=0.12,
        mpc_hold_vel_enter=0.08,
        mpc_hold_vel_exit=0.15,
        mpc_hold_kd=2.0,
        mpc_hold_torque_limit=0.6,
        torque_rate_limit=np.inf,
    ):
        # ── output directory ──────────────────────────────────
        ...
        # ── applied pendulum torques ─────────────────────────
        self.tau_profile_mode = str(tau_profile_mode).lower()
        self.tau_alpha_u0 = float(tau_alpha_u0)
        self.tau_alpha_u1 = float(tau_alpha_u1)
        self.tau_alpha_t0 = float(tau_alpha_t0)
        self.tau_alpha_t1 = float(tau_alpha_t1)
        self.tau_beta_u0 = float(tau_beta_u0)
        self.tau_beta_u1 = float(tau_beta_u1)
        self.tau_beta_t0 = float(tau_beta_t0)
        self.tau_beta_t1 = float(tau_beta_t1)
        self.torque_limit = abs(float(torque_limit))
        self.pendulum_damping = abs(float(pendulum_damping))
        self.mpc_enable = bool(mpc_enable)

        # True MPC knobs. mpc_step_distance/duration become reference
        # trajectory settings when MPC is enabled.
        self.mpc_trajectory_type = str(mpc_trajectory_type).lower()
        self.mpc_sine_amplitude = float(mpc_sine_amplitude)
        self.mpc_sine_frequency = float(mpc_sine_frequency)
        self.mpc_sine_offset = float(mpc_sine_offset)
        self.mpc_step_distance = 0.0 if mpc_step_distance is None else float(mpc_step_distance)
        self.mpc_step_duration = 8.0 if mpc_step_duration is None else max(float(mpc_step_duration), 1e-9)
        self.mpc_dt = max(float(mpc_dt), 1e-6)
        self.mpc_horizon = max(int(mpc_horizon), 2)
        self.mpc_q_pos = float(mpc_q_pos)
        self.mpc_q_vel = float(mpc_q_vel)
        self.mpc_qf_scale = float(mpc_qf_scale)
        self.mpc_r_u = float(mpc_r_u)
        self.mpc_r_du = float(mpc_r_du)
        self.mpc_guard_kp = float(mpc_guard_kp)
        self.mpc_guard_kd = float(mpc_guard_kd)
        self.mpc_guard_margin = abs(float(mpc_guard_margin))
        self.mpc_hold_enable = bool(mpc_hold_enable)
        self.mpc_hold_err_enter = abs(float(mpc_hold_err_enter))
        self.mpc_hold_err_exit = max(abs(float(mpc_hold_err_exit)), self.mpc_hold_err_enter)
        self.mpc_hold_vel_enter = abs(float(mpc_hold_vel_enter))
        self.mpc_hold_vel_exit = max(abs(float(mpc_hold_vel_exit)), self.mpc_hold_vel_enter)
        self.mpc_hold_kd = abs(float(mpc_hold_kd))
        self.mpc_hold_torque_limit = abs(float(mpc_hold_torque_limit))
        self.torque_rate_limit = abs(float(torque_rate_limit))

        # MPC-style shorthand: map (distance, duration) to a smooth-step
        # torque target with final level u1 = distance / duration.
        # The torque is always applied to the alpha DOF (pendulum revolution
        # axis) because that is the DOF that produces rolling translation.
        # The horizontal direction of motion is set by beta_p0:
        #   beta_p0 = 0      -> pendulum revolves in body x-z plane (sphere rolls in x)
        #   beta_p0 = pi/2   -> pendulum revolves in body y-z plane (sphere rolls in y)
        self.mpc_step_axis = str(mpc_step_axis).lower()
        if self.mpc_step_axis not in ("x", "y"):
            raise ValueError("mpc_step_axis must be 'x' or 'y'")
        if (not self.mpc_enable) and (mpc_step_distance is not None and mpc_step_duration is not None):
            dur = max(float(mpc_step_duration), 1e-9)
            u1 = float(mpc_step_distance) / dur
            self.tau_profile_mode = "smooth_step"
            self.tau_alpha_u0 = 0.0
            self.tau_alpha_u1 = u1
            self.tau_alpha_t0 = 0.0
            self.tau_alpha_t1 = dur
            self.tau_beta_u0 = 0.0
            self.tau_beta_u1 = 0.0
            self.tau_beta_t0 = 0.0
            self.tau_beta_t1 = dur

        if callable(tau_alpha_func):
            self.tau_alpha_func = tau_alpha_func
        else:
            self.tau_alpha_func = self._build_default_torque_profile(
                self.tau_profile_mode,
                self.tau_alpha_u0, self.tau_alpha_u1,
                self.tau_alpha_t0, self.tau_alpha_t1,
            )

        if callable(tau_beta_func):
            self.tau_beta_func = tau_beta_func
        else:
            self.tau_beta_func = self._build_default_torque_profile(
                self.tau_profile_mode,
                self.tau_beta_u0, self.tau_beta_u1,
                self.tau_beta_t0, self.tau_beta_t1,
            )

        self._mpc_u_warm = np.zeros(self.mpc_horizon)
        self._mpc_tau_hold = np.zeros(2)
        self._mpc_last_u = 0.0
        self._mpc_next_update_time = None
        self._mpc_in_hold = False

        # Effective 1D translational model used by MPC: x_ddot = b_u * u.
        # Includes translational + rotational contribution of the sphere body.
        m_eff = self.m + self.I_s / max(self.R**2, 1e-12)
        self._mpc_b_u = 1.0 / (m_eff * max(self.R, 1e-12))
        self._mpc_build_prediction_mats()
        ...
        self.tau_cmd_save = np.zeros((2, self.ntime))
    ...
    def _save_metadata(self):
        params = {
            ...
            "pendulum_damping": float(self.pendulum_damping),
            "mpc_step_axis": self.mpc_step_axis,
            "mpc_enable": bool(self.mpc_enable),
            "mpc_trajectory_type": self.mpc_trajectory_type,
            "mpc_dt": float(self.mpc_dt),
            "mpc_horizon": int(self.mpc_horizon),
            "mpc_q_pos": float(self.mpc_q_pos),
            "mpc_q_vel": float(self.mpc_q_vel),
            "mpc_qf_scale": float(self.mpc_qf_scale),
            "mpc_r_u": float(self.mpc_r_u),
            "mpc_r_du": float(self.mpc_r_du),
            "mpc_guard_kp": float(self.mpc_guard_kp),
            "mpc_guard_kd": float(self.mpc_guard_kd),
            "mpc_guard_margin": float(self.mpc_guard_margin),
            "mpc_hold_enable": bool(self.mpc_hold_enable),
            "mpc_hold_err_enter": float(self.mpc_hold_err_enter),
            "mpc_hold_err_exit": float(self.mpc_hold_err_exit),
            "mpc_hold_vel_enter": float(self.mpc_hold_vel_enter),
            "mpc_hold_vel_exit": float(self.mpc_hold_vel_exit),
            "mpc_hold_kd": float(self.mpc_hold_kd),
            "mpc_hold_torque_limit": float(self.mpc_hold_torque_limit),
            "torque_rate_limit": float(self.torque_rate_limit),
    ...
    def save_arrays(self):
        ...
        np.save(f'{self.output_path}/tau_cmd_save.npy', self.tau_cmd_save)
    ...
    @staticmethod
    def _smooth_step(t, t0, t1, y0, y1):
        if t <= t0:
            return y0
        if t >= t1:
            return y1
    ...
    def _build_default_torque_profile(self, mode, u0, u1, t0, t1):
        if mode == "smooth_step":
            return lambda t: self._smooth_step(t, t0, t1, u0, u1)
        return lambda t: 0.0

    def _x_ref_mpc(self, t):
        if self.mpc_trajectory_type == "sinusoid":
            w = 2.0 * np.pi * self.mpc_sine_frequency
            return self.mpc_sine_offset + self.mpc_sine_amplitude * np.sin(w * t)
        tt = min(float(t), self.mpc_step_duration)
        return self.mpc_step_distance * (1.0 - np.cos(np.pi * tt / self.mpc_step_duration)) / 2.0

    def _xd_ref_mpc(self, t):
        if self.mpc_trajectory_type == "sinusoid":
            w = 2.0 * np.pi * self.mpc_sine_frequency
            return self.mpc_sine_amplitude * w * np.cos(w * t)
        if t <= self.mpc_step_duration:
            return self.mpc_step_distance * (np.pi / self.mpc_step_duration) * np.sin(np.pi * t / self.mpc_step_duration) / 2.0
        return 0.0

    def _mpc_build_prediction_mats(self):
        dt = self.mpc_dt
        N = self.mpc_horizon

        self._mpc_Ad = np.array([[1.0, dt],
                                 [0.0, 1.0]])
        self._mpc_Bd = np.array([[0.5 * dt * dt * self._mpc_b_u],
                                 [dt * self._mpc_b_u]])
        ...
        Apow = np.eye(2)
        for k in range(N):
            Apow = self._mpc_Ad @ Apow
            Sx[2*k:2*(k+1), :] = Apow
            for i in range(k + 1):
                Su[2*k:2*(k+1), i:i+1] = np.linalg.matrix_power(self._mpc_Ad, k - i) @ self._mpc_Bd

        self._mpc_Sx = Sx
        self._mpc_Su = Su

        Qy = np.diag([self.mpc_q_pos, self.mpc_q_vel])
        Qf = self.mpc_qf_scale * Qy
        Qblk = [Qy] * (N - 1) + [Qf]
        self._mpc_Qbig = np.block([
            [Qblk[i] if i == j else np.zeros((2, 2)) for j in range(N)]
            for i in range(N)
        ])
        self._mpc_Ru = self.mpc_r_u * np.eye(N)
        D = np.eye(N)
        D[1:, :-1] -= np.eye(N - 1)
        self._mpc_D = D
        self._mpc_bounds = [(-self.torque_limit, self.torque_limit)] * N

    def _mpc_planar_state(self, q, u):
        if self.mpc_step_axis == "x":
            return np.array([float(q[0]), float(u[0])]), 6, q[7], u[6]
        # y-axis: track y position/velocity but still drive alpha (DOF 6).
        # beta_p0=pi/2 already places the pendulum in the y-z plane; swinging
        # alpha (not beta) is what produces rolling translation in y.
        return np.array([float(q[1]), float(u[1])]), 6, q[7], u[6]

    def _mpc_solve_step(self, z_cur, u_prev, u_warm, ref_y_seq):
        Y_const = self._mpc_Sx @ z_cur
        e0 = np.zeros(self.mpc_horizon)
        e0[0] = u_prev

        def cost(u_seq):
            Y = self._mpc_Su @ u_seq + Y_const
            eY = Y - ref_y_seq
            du = self._mpc_D @ u_seq - e0
            return float(eY @ self._mpc_Qbig @ eY + u_seq @ self._mpc_Ru @ u_seq + self.mpc_r_du * du @ du)

        def jac(u_seq):
            Y = self._mpc_Su @ u_seq + Y_const
            eY = Y - ref_y_seq
            du = self._mpc_D @ u_seq - e0
            return 2.0 * (self._mpc_Su.T @ (self._mpc_Qbig @ eY) + self._mpc_Ru @ u_seq + self.mpc_r_du * (self._mpc_D.T @ du))
        ...
        if result.success:
            return result.x
        return np.clip(u_warm, -self.torque_limit, self.torque_limit)

    def _compute_mpc_torque(self, iter_idx, q, u):
        z_cur, pend_u_idx, _, _ = self._mpc_planar_state(q, u)
        t_now = self.t[iter_idx]

        ref_y_seq = np.zeros(2 * self.mpc_horizon)
        for j in range(self.mpc_horizon):
            tj = t_now + (j + 1) * self.mpc_dt
            ref_y_seq[2*j] = self._x_ref_mpc(tj)
            ref_y_seq[2*j + 1] = self._xd_ref_mpc(tj)

        u_seq = self._mpc_solve_step(z_cur, self._mpc_last_u, self._mpc_u_warm, ref_y_seq)
        self._mpc_u_warm = np.concatenate((u_seq[1:], [u_seq[-1]]))

        x_ref_now = self._x_ref_mpc(t_now)
        e_pos = float(z_cur[0] - x_ref_now)
        v_pos = float(z_cur[1])

        if self.mpc_hold_enable:
            if self._mpc_in_hold:
                if (abs(e_pos) > self.mpc_hold_err_exit) or (abs(v_pos) > self.mpc_hold_vel_exit):
                    self._mpc_in_hold = False
            else:
                if (abs(e_pos) < self.mpc_hold_err_enter) and (abs(v_pos) < self.mpc_hold_vel_enter):
                    self._mpc_in_hold = True

        if self._mpc_in_hold:
            # In terminal hold, apply gentle damping only.
            u_cmd = -self.mpc_hold_kd * v_pos
            if np.isfinite(self.mpc_hold_torque_limit) and self.mpc_hold_torque_limit > 0.0:
                u_cmd = float(np.clip(u_cmd, -self.mpc_hold_torque_limit, self.mpc_hold_torque_limit))
        else:
            u_guard = 0.0
            # Safety brake: if we are outside the margin and still moving away
            # from the reference, add damping toward the target.
            if abs(e_pos) > self.mpc_guard_margin and (e_pos * v_pos) > 0.0:
                u_guard = -self.mpc_guard_kp * e_pos - self.mpc_guard_kd * v_pos
            u_cmd = u_seq[0] + u_guard

        u_apply = smooth_limit(u_cmd, self.torque_limit)
        if np.isfinite(self.torque_rate_limit):
            max_delta = self.torque_rate_limit * self.mpc_dt
            u_apply = np.clip(u_apply, self._mpc_last_u - max_delta, self._mpc_last_u + max_delta)
        self._mpc_last_u = float(u_apply)
    ...
    def _update_control_hold(self, iter_idx, q, u):
        if not self.mpc_enable:
            return
        t_now = self.t[iter_idx]
        if self._mpc_next_update_time is None:
            self._mpc_next_update_time = t_now
        if t_now + 1e-12 >= self._mpc_next_update_time:
            self._mpc_tau_hold = self._compute_mpc_torque(iter_idx, q, u)
            self._mpc_next_update_time = t_now + self.mpc_dt

    def _evaluate_pendulum_torques(self, iter, q, u):
        """Evaluate applied pendulum torques at current time/state.
        ...
        if self.mpc_enable:
            tau = np.zeros(self.nu, dtype=q.dtype)
            tau[6] = self._mpc_tau_hold[0]
            tau[7] = self._mpc_tau_hold[1]
            if self.pendulum_damping > 0.0:
                tau[6] = tau[6] - self.pendulum_damping * u[6]
                tau[7] = tau[7] - self.pendulum_damping * u[7]
            return tau

        def _call(fun):
            try:
                return float(fun(t_now, q, u))
            except TypeError:
                try:
                    return float(fun(t_now, q))
                except TypeError:
                    return float(fun(t_now))

        tau = np.zeros(self.nu, dtype=q.dtype)
        tau[6] = np.clip(_call(self.tau_alpha_func), -self.torque_limit, self.torque_limit)
        tau[7] = np.clip(_call(self.tau_beta_func), -self.torque_limit, self.torque_limit)
        # Viscous damping on pendulum DOFs (subtracted from applied torque)
        # to prevent unbounded spin-up from internal reaction torque coupling.
        if self.pendulum_damping > 0.0:
            tau[6] = tau[6] - self.pendulum_damping * u[6]
            tau[7] = tau[7] - self.pendulum_damping * u[7]
        return tau
    ...
    def get_R(self, iter, X, prev_X, prev_AV, prev_q, prev_u, prev_gNdot, prev_gammaF):
        ...
        tau = self._evaluate_pendulum_torques(iter, q, u)
        ...
        # Smooth residual
        Rs = np.concatenate((
            M @ a + h - Wg @ lambda_g - Wgamma @ lambda_gamma - WN @ lambdaN - WF @ lambdaF - tau,
    ...
    def time_update(self, iter):
        ...
        # Receding-horizon control is evaluated once per major time step and
        # held constant during the Newton solves of this step.
        self._update_control_hold(iter, prev_q, prev_u)
        ...
        self.tau_cmd_save[:, iter] = self._mpc_tau_hold
        self.save_arrays()
...
if __name__ == "__main__":
    # Mirrors the hoop_pendulum_controller MPC example: the pendulum is given
    # an initial spin so it revolves, and the torque limit is high enough
    # that the smooth-step torque can sustain that revolution.
    mpc_step_distance = 4.0
    mpc_step_duration = 8.0
    mpc_step_axis = "y"             # 'x' or 'y' (horizontal rolling axis)
    torque_limit = 4.0              # reduced to curb acceleration and overshoot
    torque_rate_limit = 10.0        # slower actuator slew for realism
    pendulum_damping = 0.2          # stronger damping reduces unrealistic spin-up

    # Pendulum IC selects the plane of revolution and therefore the rolling
    # direction. The torque always drives alpha (revolution angle).
    alpha_p0 = -np.pi/2                              # upright start
    beta_p0 = 0.0 if mpc_step_axis == "x" else np.pi / 2.0  # plane selector
    alpha_p_dot0 = 0.0                                     # gentler initial spin
    ...
    sim = Simulation(
        ...
        x0=-0.5, y0=-0.5, z0=0.18,
        p0_0=1.0, p1_0=0.0, p2_0=0.0, p3_0=0.0,
        alpha_p0=alpha_p0, beta_p0=beta_p0,
        alpha_p_dot0=alpha_p_dot0,  # give pendulum initial spin
        n_stairs=0, stair_width=1.8, stair_height=0.1,
        stair_x_start=0.5, fillet_radius=0,
        # Define only these two, as in hoop controller style.
        mpc_step_distance=mpc_step_distance,
        mpc_step_duration=mpc_step_duration,
        mpc_step_axis=mpc_step_axis,
        mpc_enable=True,
        mpc_trajectory_type="smooth_step",
        mpc_dt=0.05,
        mpc_horizon=22,
        mpc_q_pos=6.0,
        mpc_q_vel=5.0,
