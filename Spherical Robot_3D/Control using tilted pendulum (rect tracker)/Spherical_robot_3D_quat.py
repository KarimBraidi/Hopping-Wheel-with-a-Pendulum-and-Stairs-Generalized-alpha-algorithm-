...
class Simulation:
    ...
    def _pendulum_control_torque(self, q, u):
        ...
        Cascade:
          ...
          4. virtual spring-damper at the pendulum tip mapped to joint torques
             via J^T.  This naturally tracks a WORLD-frame goal while the
             sphere rolls (the tip is the slow variable).
          5. The joint damper is a passive sink that prevents that runaway 
             without fighting the spatial goal.
          6. saturate at +-tau_max.
        """
        if not self.control_enabled:
            return np.zeros(2)

        # outer loops
        W = self.waypoints[self.wp_idx]
        err = W - q[0:2]
        dist = np.linalg.norm(err)
        if dist > 1e-12:
            v_des = min(self.v_max, self.k_pos * dist) * (err / dist)
        else:
            v_des = np.zeros(2)
    ...
    def _advance_waypoint(self, q):
        """Switch to the next rectangle corner once close enough (called once
        per accepted time step, never inside Newton)."""
        if not self.control_enabled or self.wp_idx >= len(self.waypoints) - 1:
            return
        if np.linalg.norm(self.waypoints[self.wp_idx] - q[0:2]) < self.waypoint_tol:
            self.wp_idx += 1
...
if __name__ == "__main__":
    ...
    sim = Simulation(
        ...
        control_rect=(4.0, 1.5),  # Lx x Ly [m]
        tau_max=1.0,              # [N*m] 
