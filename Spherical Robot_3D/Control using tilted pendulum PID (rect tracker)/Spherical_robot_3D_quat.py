...
class Simulation:
    def __init__(
        ...
        # torque-controlled pendulum (rectangle tracking), all inputs in SI
        ...
        v_max=0.4,                # cruise speed along rectangle [m/s]
        ...
        # ── torque-controlled pendulum (rectangle tracking) ───
        self.control_enabled = control_rect is not None
        ...
        if self.control_enabled:
            ...
            # corners traversed counter-clockwise, ending back at start
    ...
    # ══════════════════════════════════════════════════════════
    #  Pendulum torque controller (rectangle tracking)
    # ══════════════════════════════════════════════════════════
    def _pendulum_control_torque(self, q, u):
        """Joint torques (tau_alpha, tau_beta) in code units.

        Cascade:
          1. waypoint -> desired sphere velocity (capped at v_max)
        ...
        # outer loops: follow the active rectangle edge, with a separate
        # damped cross-track correction. This avoids aiming through each
        # corner and reduces lateral hunting on short rectangle sides.
        ...
        if edge_len > 1e-12:
            t_hat = edge / edge_len
            s = (q[0:2] - A) @ t_hat
            closest = A + np.clip(s, 0.0, edge_len) * t_hat
    ...
    def _advance_waypoint(self, q):
        """Switch to the next rectangle edge after passing its end."""
        ...
        if progress >= edge_len and np.linalg.norm(B - q[0:2]) < self.waypoint_tol:
            self.wp_idx += 1
    ...
    def plot_results(self):
        ...
        if self.control_enabled:
            ...
            rect = np.vstack([[self.q_save[0, 0], self.q_save[1, 0]], self.waypoints])
            ax.plot(rect[:, 0], rect[:, 1], 'k--', linewidth=1.0, label='rectangle')
...
if __name__ == "__main__":
    # Torque-controlled pendulum: sphere tracks a rectangle on flat ground.
    sim = Simulation(
        ntime=6000,
        dt=4e-3,
        mu_s=10**9, mu_k=0.5,
